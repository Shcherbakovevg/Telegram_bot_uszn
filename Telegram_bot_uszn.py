...
async def day_chosen(message: types.Message, state: FSMContext):
	...
	if message.text.lower() == messages.back:
		...
		try:
			await state.update_data(day_list=db.get_days(messages.MonthId[(user_data['chosen_month']).title()],\
...
async def time_chosen(message: types.Message, state: FSMContext):
	...
	if message.text.lower() == messages.back:
		...
		user_data = await state.get_data()
		try:
			await state.update_data(periods_list=db.get_free_periods (messages.MonthId[(user_data['chosen_month']).title()],\
...
async def name_chosen(message: types.Message, state: FSMContext):
	...
	if message.text.lower() == messages.back:
		...
		try:
			user_data = await state.get_data()
			db.rezerv_time(messages.MonthId[(user_data['chosen_month']).title()], user_data['chosen_day'],\
...
async def data_accept(message: types.Message, state: FSMContext):
	if message.text.lower() == messages.back:
		...
		user_data = await state.get_data()
		try:
			await state.update_data(OrderId=db.get_client_num(messages.MonthId[(user_data['chosen_month']).title()],\
